<script lang="ts">
  ...
  function insertStop(index: number) {
    ...
    if (index >= parsedStops.length)
      throw RangeError("Cannot insert stop after last stop");
    ...
    parsedStops = [
      ...parsedStops.slice(0, index),
      [color, position],
      ...parsedStops.slice(index),
    ];
  }
</script>
...
<div class="column" style:width>
  ...
  {#each parsedStops as [color, position], index}
    <GradientStop
      ...
      onremove={() => (parsedStops = parsedStops.filter((_, i) => i != index))}
      editable={true}
    />
  {/each}
  <div class="rowreverse">
    <IconButton
      hasBackground
      isSmall
      onclick={() => insertStop(parsedStops.length - 1)}
    >
